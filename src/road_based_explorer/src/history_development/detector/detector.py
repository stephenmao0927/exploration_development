...
from rrt_exploration.msg import RoboPointArray

costmap_topic = rospy.get_param('~costmap_topic', default='/robot_1/move_base_node/global_costmap/costmap')
# map_frame =  rospy.get_param('~map_frame',default='robot_1/map')
odom_topic = rospy.get_param('~odometry_topic',default='/robot_1/Odometry')
# odom_frame = rospy.get_param('~odometry_frame',default='robot_1/body')
radius = rospy.get_param('~radius', default = 60) #采样范围半径，单位：格
number = rospy.get_param('~number', default = 40) #采样点数目
...
class Pick_Points(object):
    def __init__(self, costmap_data, map_pose, map_size):
        ...
        self.final_world = self.WorldCoordinate(costmap_data, self.final)
        # 获取筛选完后的最终采样点
    ...
    def sampling(self):
        ...
        while i < (len(self.circle_point_value)) and j < (len(self.circle_point_value)):
            if self.circle_point_value[int(j)][2] == 0:
                self.first_pick.append(self.circle_point[int(j)])
            i += self.step
            j = round(i) 
        ...
        while self.circle_point_value[int(j)][2] == 0:
            self.delete.append(self.circle_point[int(j)])
            i -= self.step
            j = round(i)
        ...
        self.final = list(set(self.first_pick) - set(self.delete)) 

class Publish(object):
    def __init__(self, world_list):
        self.marker1 = Marker()
        self.arraypoints = RoboPointArray()
        self.world_list = world_list
        pub = rospy.Publisher('/marker_detected_points', Marker, queue_size=10)
        pub2 = rospy.Publisher('/detected_points', RoboPointArray, queue_size=10)
        ...
        for q in self.world_list:
            ...
            point.y = q[1]  # 设置点的y坐标
            point.z = 0.0 
            self.arraypoints.points.append(point)
        ...
        self.arraypoints.name = "robot_1"

        pub.publish(self.marker1)
        pub2.publish(self.arraypoints)
        rate.sleep()
