...
def is_stuck():
	global robot_name, current_goal_id, robot_state, history_position, stuck_count
...
#-------------------------------------------------------------------------	
	...
	# print(stuck_count)
	if stuck:
		rospy.set_param('~/'+ robot_name +'/is_stuck','true')
	
	# if robot_name == "robot_1":
	# 	print(robot_name + " ready to send ? ", ready_to_send)

	if len(id_record) > 0 and len(centroids) > 0 and (ready_to_send or stuck):
		winner_id=revenue_record.index(max(revenue_record))
		# rospy.loginfo(max(revenue_record))
		# rospy.loginfo(revenue_record)
		robots[id_record[winner_id]].sendGoal(centroid_record[winner_id])
		stuck_count = 0
		rospy.set_param('~/'+ robot_name +'/is_stuck','false')
		# check_far = True
		# waiting_count = 0
		...
		print("new goal for "+ robot_name +" sent with ready to send: "+str(ready_to_send)+" and stuck: "+str(stuck))
		ready_to_send = False
...
def node():
	global frontiers, current_goal_id, robot_state, history_position, stuck_count, globalmaps, current_goal, plot, loop, n_robots, robots, robot_name, corner, delay_after_assignement, ready_to_send
